...
function generateOrg3Definition() {
  ...
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.food-network.com/org3.json
   res=$?
   set +x
   if [ $res -ne 0 ]; then
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.food-network.com" ]; then
    generateOrg3
